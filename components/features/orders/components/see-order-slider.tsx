...
const SeeOrderSlider: React.FC<SeeOrderSliderProps> = ({ 
  isOpen, 
  onClose, 
  order,
  onSliderStateChange,
  ...
  const orderData: OrderData = order ? {
    ...
    phone: "",
    orderDate: order.date ? formatOrderDate(order.date) : "N/A",
    ...
    activities: order.activity_log && order.activity_log.length > 0 
      ? order.activity_log.map((activity: ActivityLog) => {
                  ...
                  <div key={item.id}>
                    <div className="flex items-start py-[8px] px-[8px]">
                      ...
                      <div className="flex items-start space-x-[8px] flex-[4]">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="body-3 text-gray-500 font-urbanist text-sm">
                              <span className='text-black font-urbanist'>₹{(item.unitPrice || 0).toLocaleString()}</span>/unit  {/*<span className='text-secondary-900'>• </span> {item.weight}  <span className='text-secondary-900'>• </span> Color: {item.color}*/}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="text-right ml-4 flex-[1]">
                        <div className="body-3 text-gray-900 font-urbanist font-bold">₹{(item.subtotal || 0).toLocaleString()}</div>
                ...
                <div className="py-[16px] px-[16px] space-y-6">
                  {orderData.items.map((item, itemIndex) => {
                    ...
                    const getCurrentStatusLabel = () => {
                      ...
                      if (itemProgress.packaging === 'in-progress') return 'Pending Packaging'
                        ...
                        <div className="relative mb-4">
                          ...
                          <div className="relative flex justify-between">
                            ...
                            <div className="flex flex-col items-center">
                              <div className={cn(
                                "w-6 h-6 rounded-full flex items-center justify-center border-2 bg-white",
                                itemProgress.packaging === 'completed' ? "border-green-500" : 
                                itemProgress.packaging === 'in-progress' ? "border-yellow-500" : "border-gray-300"
                              )}>
                                {itemProgress.packaging === 'completed' && <CheckCircle className="h-4 w-4 text-green-500" />}
                                {itemProgress.packaging === 'in-progress' && <AlertCircle className="h-4 w-4 text-yellow-500" />}
                                {itemProgress.packaging === 'pending' && <span className="text-xs font-semibold text-gray-400">3</span>}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className={cn(
                                ...
                                {itemProgress.pkgQc === 'pending' && !(item.packagingQcStatus && item.packagingQcStatus.toLowerCase() === 'rejected') && <span className="text-xs font-semibold text-gray-400">4</span>}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className={cn(
                                ...
                                {itemProgress.shipped === 'pending' && <span className="text-xs font-semibold text-gray-400">5</span>}
                              </div>
                            </div>
                          </div>
                        </div>
             ...
             <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
               ...
               <div className="overflow-hidden">
                 ...
                 <div className="grid grid-cols-5 bg-white">
                   ...
                   <div className="px-[8px] py-[4px] border-r border-gray-200">
                      <div className="text-base font-semibold text-gray-900 font-urbanist whitespace-nowrap">₹{(orderData.summary.subtotalAmount || 0).toLocaleString()}</div>
                   </div>
                   <div className="px-[8px] py-[4px] border-r border-gray-200">
                     <div className="text-base font-semibold text-red-600 font-urbanist whitespace-nowrap">₹{(orderData.summary.taxAmount || 0).toLocaleString()}</div>
                   </div>
                   <div className="px-[8px] py-[4px]">
                     <div className="text-base font-semibold text-green-600 font-urbanist whitespace-nowrap">₹{(orderData.summary.totalAmount || 0).toLocaleString()}</div>
                   </div>
                 </div>
               </div>
             </div>
                      ...
                      {orderData.activities.map((activity, index) => (
                        <div key={index} className="relative flex items-start">
                          ...
                          <div className="ml-4 flex-1">
                            ...
                            {/* QC Status Badge */}
                            {(() => {
                              const qcBadge = getQcStatusBadge((activity as any).qcStatus)
                              return qcBadge ? (
                                <div className="mt-1">
                                  <span
                              ...
                              <div className="mt-2 flex gap-2 flex-wrap">
                                {activity.images.map((img, imgIndex) => (
                                  <img 
                                    key={imgIndex}
                                    src={img} 
                                    alt={`Activity ${index} Image ${imgIndex + 1}`}
