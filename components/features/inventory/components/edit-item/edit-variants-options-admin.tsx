...
export const ProductOptionsVariantsAdmin = ({
  options,
  variants,
  apiVariants,
  variantOptions,
  ...
  const [createdVariants, setCreatedVariants] = useState<
    Array<{
      ...
      stock: number;
      media?: File[];
      apiId?: number; // Store API variant ID
  ...
  const [newVariantSelections, setNewVariantSelections] = useState<
    Record<string, string>
  >({});

  // Additional variant details modal state
  const [isAdditionalDetailsOpen, setIsAdditionalDetailsOpen] = useState(false);
  const [selectedAdditionalDetailsIndex, setSelectedAdditionalDetailsIndex] =
    useState<number | null>(null);
  const [variantImageErrors, setVariantImageErrors] = useState<
    Record<number, boolean>
  >({});

  // Search and caret state
  const [searchQuery, setSearchQuery] = useState("");
  const [isVariantsTableCollapsed, setIsVariantsTableCollapsed] =
    useState(false);

  // Duplicate variant modal state
  const [isDuplicateModalOpen, setIsDuplicateModalOpen] = useState(false);
  const [duplicateVariantIndex, setDuplicateVariantIndex] = useState<number | null>(null);
  const [duplicateVariantSelections, setDuplicateVariantSelections] = useState<Record<string, string>>({});
  
  // Track upload status for each variant
  ...
  // Store extra charges by variant combination key (to preserve when variant is deleted and recreated)
  const [extraChargesByCombination, setExtraChargesByCombination] = useState<
  ...
  React.useEffect(() => {
    // Prioritize API image URL, then uploaded file, then default image
    if (productImageUrl) {
      ...
      // Cleanup: revoke the URL when component unmounts or image changes
  ...
  // Update parent with variants data whenever createdVariants changes
  React.useEffect(() => {
    const updateVariants = () => {
      const validRows = attributeRows.filter(
        (row) =>
      ...
      if (createdVariants.length > 0 && validRows.length > 0) {
        ...
        const formattedVariants = createdVariants.map((variant, variantIndex) => {
          // Create title from attribute values (e.g., "Red/Large")
          const titleParts = validRows.map(
            (row) => variant[row.attributeType] || "N/A"
          );
          const title = titleParts.join("/");

          // Use already uploaded image URLs (uploaded immediately when selected)
          const images = variantImageUrls[variantIndex] || [];

          // Check if variant has at least one image
          if (images.length === 0) {
            errors[variantIndex] = true;
          }
          ...
          return {
            ...
            variant_product_dimension_unit:
              variant.variantDimensionUnit || "cm",
        ...
        // Clear variants when all are removed or when there are no valid attribute rows
        onVariantsChange([]);
    ...
    updateVariants();
  }, [createdVariants, attributeRows, variantImageUrls]);
  ...
  React.useEffect(() => {
    ...
    const hasInvalidVariants = createdVariants.some((variant) => {
      ...
      const variantAttributeTypes = Object.keys(variant).filter(
        (key) =>
          ![
            "id",
            "productName",
      ...
      const hasRemovedAttributes = variantAttributeTypes.some(
        (attrType) => !validAttributeTypes.has(attrType)
      );
    ...
    if (hasInvalidVariants) {
      ...
      if (
        cleanedVariants.length !== createdVariants.length ||
        cleanedVariants.some((v, i) => {
          ...
          const cleanedKeys = Object.keys(v).filter(
            (k) =>
              ![
                "id",
                "productName",
        ...
        setExtraChargeInputs((prev) => {
          ...
          cleanedVariants.forEach((variant, newIndex) => {
            ...
            const oldIndex = createdVariants.findIndex(
              (v) => v.id === variant.id
            );
  ...
  // Populate table from API variants data
  React.useEffect(() => {
    if (apiVariants && apiVariants.length > 0) {
      // Extract unique option names from the first variant
      const optionNames = apiVariants[0].options.map((opt) => opt.name);

      // Create attribute rows from option names
      const newAttributeRows = optionNames.map((name, index) => ({
        id: `api-${index}`,
        attributeType: name,
        attributeValues: "",
      }));

      // Extract all unique values for each option
      optionNames.forEach((optionName, index) => {
        ...
        apiVariants.forEach((variant) => {
          ...
          if (option) {
            uniqueValues.add(option.values);
          }
        });
        newAttributeRows[index].attributeValues =
          Array.from(uniqueValues).join(", ");
      });

      setAttributeRows(newAttributeRows);

      // Store image URLs from API (now images are directly on variant object)
      const imageUrlsMap: Record<number, string[]> = {};
      apiVariants.forEach((variant, index) => {
        if (variant.images && variant.images.length > 0) {
          imageUrlsMap[index] = variant.images;
        }
      });
      setVariantImageUrls(imageUrlsMap);

      // Convert API variants to createdVariants format
      const formattedVariants = apiVariants.map((variant, index) => {
        const dynamicAttributes: Record<string, string> = {};
        variant.options.forEach((option) => {
          dynamicAttributes[option.name] = option.values;
        });

        const variantData = {
          id: `D${String(index + 1).padStart(3, "0")}`,
          productName: variant.name || productName || "Product",
          ...dynamicAttributes,
          extraCharge: variant.extra_charges || 0,
          stock: variant.stock_quantity || 0,
          apiId: variant.id, // Store the API ID for reference
          isActive: variant.is_active !== undefined ? variant.is_active : true, // Store active status
        ...
        // Save extra charge for this combination
        if (variantData.extraCharge && variantData.extraCharge > 0) {
          const combinationKey = getVariantCombinationKey(variantData, newAttributeRows);
          setExtraChargesByCombination((prev) => ({
            ...prev,
            [combinationKey]: variantData.extraCharge,
          }));
        }
      ...
      setCreatedVariants(formattedVariants);
      setShowVariantsTable(true);

      // Create options data
      const optionsData: Option[] = newAttributeRows.map((row) => ({
        name: row.attributeType,
        values: row.attributeValues
          .split(",")
          .map((v) => v.trim())
          .filter((v) => v),
      }));
  ...
  React.useEffect(() => {
    ...
    if (
      showPreviewVariants &&
      (validRows.length < 2 || attributeRows.length === 0)
    ) {
  ...
  const updateAttributeRow = (
    id: string,
    field: "attributeType" | "attributeValues",
    value: string
  ) => {
    ...
    setAttributeRows((prevRows) =>
      prevRows.map((row) => {
        if (row.attributeType) {
          ...
          const isStillAvailable = availableTypes.some(
            (option) => option.name === row.attributeType
          );
  ...
  const togglePresetValueToAttribute = (rowId: string, valueName: string) => {
    setAttributeRows(
      attributeRows.map((row) => {
        if (row.id === rowId) {
          const currentValues = row.attributeValues
            ? row.attributeValues
                .split(",")
                .map((v) => v.trim())
                .filter((v) => v)
  ...
  // Handle opening duplicate variant modal
  const handleOpenDuplicateModal = () => {
    // Reset selections to empty
    setDuplicateVariantSelections({});
    setDuplicateVariantIndex(null);
    setIsDuplicateModalOpen(true);
  };

  // Handle closing duplicate variant modal
  ...
  // Handle adding duplicate variant with selected attributes
  const handleAddDuplicateVariant = () => {
    const validRows = attributeRows.filter(
      (row) =>
        row.attributeType && row.attributeValues && row.attributeValues.trim()
    );
    ...
    const allAttributesSelected = validRows.every(
      (row) =>
        duplicateVariantSelections[row.attributeType] &&
        duplicateVariantSelections[row.attributeType].trim()
    );
    ...
    const maxId = createdVariants.reduce((max, v) => {
      const numMatch = v.id?.match(/D(\d+)/);
      if (numMatch) {
    ...
    // Create dynamic attributes object from selections
    const dynamicAttributes: Record<string, string> = {};
    validRows.forEach((row) => {
      dynamicAttributes[row.attributeType] = duplicateVariantSelections[row.attributeType];
    });

    // Check if there's a saved extra charge for this combination
    const tempVariant = { ...dynamicAttributes };
    const combinationKey = getVariantCombinationKey(tempVariant, validRows);
    const savedExtraCharge = extraChargesByCombination[combinationKey] || 0;

    // Create new variant with selected attributes and default values
    const newVariant = {
      id: `D${String(maxId + 1).padStart(3, "0")}`,
      productName: productName || "Product",
      ...dynamicAttributes,
      extraCharge: savedExtraCharge, // Restore saved extra charge if available
      stock: 0,
      isActive: true,
      extraLeadTime: 0,
      variantLength: 0,
  ...
  const handleAddNewVariant = () => {
    ...
    const allAttributesSelected = validRows.every(
      (row) =>
        newVariantSelections[row.attributeType] &&
        newVariantSelections[row.attributeType].trim()
    );
    ...
    const variantParts = validRows.map(
      (row) => newVariantSelections[row.attributeType]
    );
    const newVariantCombination = variantParts.join(" x ");
    const variantExistsInTable = variantCombinations.includes(
      newVariantCombination
    );
    ...
    // Check if this variant already exists in createdVariants
    const variantKey = variantParts.join("|");
    const existingVariant = createdVariants.find((v) => {
      const existingKey = validRows
        .map((row) => (v as any)[row.attributeType] || "N/A")
        .join("|");
      return existingKey === variantKey;
    });

    if (existingVariant) {
      toast.error("This variant combination already exists in the table");
      return;
    }
  ...
  const variantCombinations = allVariantCombinations.filter(
    (combo) => !excludedVariants.has(combo)
  );
  ...
  React.useEffect(() => {
    if (
      ...
      allVariantCombinations.length > 0
    ) {
      setShowPreviewVariants(false);
  ...
  const handleFileUpload = async (index: number, files: FileList | null) => {
    ...
    // Add new files to existing media array instead of replacing
    setCreatedVariants((prev) =>
      prev.map((v, i) =>
        i === index ? { ...v, media: [...(v.media || []), ...validFiles] } : v
      )
    );
    // Previews - append new URLs to existing previews
    ...
    try {
      ...
      // Add uploaded URLs to variant image URLs
      setVariantImageUrls(prev => ({
        ...prev,
        [index]: [...(prev[index] || []), ...uploadedUrls]
      }));
      // Clear local files since they're now in variantImageUrls
      setCreatedVariants(prev => prev.map((v, i) => {
        if (i !== index) return v;
    ...
    // Clear error for this variant when image is uploaded
  ...
  const handleDeleteVariantApiImage = async (variantIndex: number, imageIndex: number) => {
    const imageUrlToDelete = variantImageUrls[variantIndex]?.[imageIndex];
    if (!imageUrlToDelete) return;
    
    // Optimistically remove from UI
    const previousUrls = { ...variantImageUrls };
    setVariantImageUrls(prev => {
      const newUrls = { ...prev };
      newUrls[variantIndex] = (newUrls[variantIndex] || []).filter((_, i) => i !== imageIndex);
      return newUrls;
    });
    ...
    try {
      ...
      // Revert on error
      setVariantImageUrls(previousUrls);
      toast.error('Failed to delete variant image. Please try again.');
    }
  };
  ...
  const removePreviewAt = (index: number, previewIndex: number) => {
    setCreatedVariants((prev) => {
      const updated = prev.map((v, i) => {
        if (i !== index) return v;
        const media = (v.media || []).filter(
          (_, mIdx) => mIdx !== previewIndex
        );
        return { ...v, media };
      });

      // Check if variant has no images left after removal and set error
      const currentVariant = updated[index];
      const hasApiImages = (variantImageUrls[index] || []).length > 0;
      const hasUploadedImages = (currentVariant?.media || []).length > 0;

      if (!hasApiImages && !hasUploadedImages) {
        setVariantImageErrors((prevErrors) => ({
          ...prevErrors,
          [index]: true,
        }));
      }
    ...
    setMediaPreviews((prev) => {
      const next = { ...(prev || {}) };
      const arr = (next[index] || []).filter(
        (_, pIdx) => pIdx !== previewIndex
      );
      next[index] = arr;
      return next;
    });
  ...
  }, [variantMediaUrls, previewImageUrl]);
  ...
  const filteredVariants = React.useMemo(() => {
    ...
    return createdVariants.filter((variant) => {
      ...
      for (const row of validRows) {
        ...
        if (
          attributeValue &&
          attributeValue.toString().toLowerCase().includes(query)
        ) {
          return true;
        }
      }

      // Search in extra charge and stock
      if (
        variant.extraCharge?.toString().includes(query) ||
  ...
  // Helper function to generate a variant combination key
  const getVariantCombinationKey = (
    variant: any,
    validRows: Array<{ attributeType: string; attributeValues: string }>
  ): string => {
    return validRows
      .map((row) => (variant as any)[row.attributeType] || "N/A")
      .join("|");
  };

  // Helper function to extract attribute values from a combination string
  const extractAttributeValuesFromCombination = (
    combination: string,
    validRows: Array<{ attributeType: string; attributeValues: string }>
  ): Record<string, string> => {
    const dynamicAttributes: Record<string, string> = {};
    
    // Get all possible values for each attribute, sorted by length (longest first)
    const attributeValuesList = validRows.map((row) =>
      row.attributeValues
        .split(",")
        .map((v) => v.trim())
        .filter((v) => v)
    ...
    // Try to match the combination by finding which values were used
    // We'll match from the end to handle values that contain " x "
    let remainingCombination = combination.trim();
    
    for (let i = validRows.length - 1; i >= 0; i--) {
      const row = validRows[i];
      const possibleValues = attributeValuesList[i];
      let matched = false;
      
      // Try to find a matching value (already sorted longest first)
      for (const value of possibleValues) {
        // Check if this value appears at the end of the remaining combination
        // Use exact match to avoid partial matches
        const valueTrimmed = value.trim();
        if (remainingCombination === valueTrimmed || remainingCombination.endsWith(` x ${valueTrimmed}`)) {
          dynamicAttributes[row.attributeType] = valueTrimmed;
          // Remove this value and the " x " separator from the remaining combination
          if (remainingCombination === valueTrimmed) {
            remainingCombination = "";
          } else {
            remainingCombination = remainingCombination
              .slice(0, remainingCombination.length - valueTrimmed.length - 3) // -3 for " x "
              .trim();
          }
          matched = true;
          break;
      ...
      // If no match found, use "N/A" as fallback
      if (!matched) {
        dynamicAttributes[row.attributeType] = "N/A";
      }
    }
  ...
  const createVariantsTable = async () => {
    ...
    const newVariantsData = variantCombinations.map((combination, index) => {
      // Extract attribute values properly, handling values that contain " x "
      const dynamicAttributes = extractAttributeValuesFromCombination(
        combination,
        validRows
      );

      // Create a unique key from the combination to check if variant already exists
      const variantKey = Object.values(dynamicAttributes).join("|");
      ...
      // If variant exists, preserve its data, otherwise create new one
      if (existingVariant) {
        return existingVariant;
      }

      // Check if there's a saved extra charge for this combination
      const savedExtraCharge = extraChargesByCombination[variantKey] || 0;

      // Generate a unique ID that doesn't conflict with existing variants
      const maxId = createdVariants.reduce((max, v) => {
        const numMatch = v.id?.match(/D(\d+)/);
        if (numMatch) {
          const num = parseInt(numMatch[1], 10);
      ...
      return {
        id: `D${String(maxId + index + 1).padStart(3, "0")}`,
        productName: productName || "Product",
        ...dynamicAttributes,
        extraCharge: savedExtraCharge, // Restore saved extra charge if available
        stock: 0,
        isActive: true, // Default to active for new variants
        extraLeadTime: 0,
        variantLength: 0,
        variantWidth: 0,
        variantHeight: 0,
    ...
    // Merge new variants with existing ones, avoiding duplicates
    ...
    const variantsToAdd = newVariantsData.filter((v) => {
      const key = validRows
        .map((row) => (v as any)[row.attributeType] || "N/A")
        .join("|");
    ...
    // Combine existing variants with new ones
    setCreatedVariants([...createdVariants, ...variantsToAdd]);
    setShowVariantsTable(true);
    ...
    setShowPreviewVariants(false);
    // Reset to first page when creating new variants
    setCurrentPage(1);
  };
  ...
  return (
    <>
      ...
      {/* Main Card */}
      <Card
        className={`bg-white border rounded-lg rounded-t-none shadow-sm mx-[16px] mb-4 ${
          isVariantChange ? "border-yellow-300 bg-yellow-50" : "border-gray-200"
        }`}
      >
            ...
            <CardContent className="px-0 pb-0">
              ...
              variantCombinations.length >= 1 ? (
                <div className="space-y-4 mt-3">
                  {/* Search bar and Create Variants button */}
                  <div className="flex items-center justify-between gap-4 px-5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {/* <Button
                        ...
                        onClick={() => setIsVariantsTableCollapsed(!isVariantsTableCollapsed)}
                      >
                        {isVariantsTableCollapsed ? (
                  ...
                  <Collapsible
                    open={!isVariantsTableCollapsed}
                    onOpenChange={(open) => setIsVariantsTableCollapsed(!open)}
                  >
                    <CollapsibleContent>
                          ...
                          <table className="w-full">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {paginatedVariants.map((variant, index) => {
                                // Find the actual index in createdVariants array by matching the variant id
                                const actualIndex = createdVariants.findIndex(
                                  (v) => v.id === variant.id
                                );
                                // Skip rendering if variant not found (shouldn't happen, but safety check)
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                      <Input
                                        ...
                                        onBlur={() => {
                                          ...
                                          // Update the variant and save extra charge
                                          setCreatedVariants((prev) => {
                                            const updated = prev.map((v, i) =>
                                              i === actualIndex
                                                ? { ...v, extraCharge: numValue }
                                            ...
                                            const variant = updated[actualIndex];
                                            if (variant) {
                                              const validRows = attributeRows.filter(
                                                (row) =>
                                                  row.attributeType &&
                                              ...
                                              if (numValue > 0) {
                                                setExtraChargesByCombination((prevCharges) => ({
                                                  ...prevCharges,
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                      <Input
                                        ...
                                        onChange={(e) => {
                                          const value = e.target.value;
                                          // Only allow numbers (no decimal for stock)
                                          if (
                                            value === "" ||
                                            /^\d+$/.test(value)
                                          ) {
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                      ...
                                      (variantImageUrls[actualIndex] &&
                                        variantImageUrls[actualIndex].length >
                                          0) ? (
                                        <button
                                          type="button"
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                      <div className="flex items-center gap-2">
                                        <Button
                                          ...
                                          onClick={() => {
                                            setSelectedAdditionalDetailsIndex(
                                              actualIndex
                                            );
                                            setIsAdditionalDetailsOpen(true);
                                          }}
                                        >
                                          <CirclePlus className="h-3 w-3 text-gray-600" />
                                        </Button>
                                        <Button
                                          type="button"
                                          variant="outline"
                                          size="sm"
                                          className="h-6 w-6 p-0 border-gray-300 hover:bg-red-50"
                                          onClick={() => {
                                            // Get the variant being deleted
                                            const variantToDelete = createdVariants[actualIndex];
                                            
                                            // Save the extra charge for this combination before deleting
                                            if (variantToDelete) {
                                              const validRows = attributeRows.filter(
                                                (row) =>
                                                  row.attributeType &&
                                              ...
                                              if (variantToDelete.extraCharge && variantToDelete.extraCharge > 0) {
                                                setExtraChargesByCombination((prev) => ({
                                                  ...prev,
                                                  [combinationKey]: variantToDelete.extraCharge,
                                                }));
                                              }
                                            }

                                            // Remove the variant
                                            const updatedVariants = createdVariants.filter(
                                              (_, i) => i !== actualIndex
                                            );
                                            setCreatedVariants(updatedVariants);

                                            // Also remove from variantImageUrls and reindex
                                            setVariantImageUrls((prev) => {
                ...
                <div className="text-center py-[30px]">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-8 px-3 font-medium bg-secondary-900 hover:bg-secondary-800 text-white hover:text-white mt-[16px]"
                    onClick={() => setIsSlideOpen(true)}
                  >
                    Create Variants
                  </Button>
                      ...
                      return validRows.map((row, index) => {
                        ...
                        return (
                          <div
                            ...
                            className={`${
                              index % 2 === 0
                                ? "border-r border-gray-200 pr-4 border-t border-b px-[8px]"
                                : "border-t border-b px-[8px]"
                            }`}
                          >
            ...
            <div className="bg-white rounded-lg border border-gray-200">
              ...
              <div className="flex items-start py-[8px] px-[8px]">
                      ...
                      <div className="text-right ml-4 flex-[1]">
                        <div className="body-3 text-gray-900 font-spectral font-bold">{formatIndianCurrency(productPrice || 0)}</div>
                    ...
                    <div className={cn(
                      ...
                      {hasValidAttributesForPreview() && (
                        <Button 
                          ...
                          className="h-6 bg-white hover:bg-white text-secondary-900 hover:text-secondary-800 border border-secondary-900 px-2 py-4 rounded-lg ml-4"
                        >
                          Preview Variant
                        </Button>
                      )}
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2 p-[8px]">
                      {variantCombinations.map((combination, index) => (
                        ...
                        >
                          <span className="body-3 font-urbanist text-natural-800">
                            {combination}
                          </span>
                          <button
                            type="button"
                            onClick={() =>
                              removeVariantCombination(combination)
                            }
                      ...
                      >
                        <div className="grid grid-cols-2 gap-4">
                          {(() => {
                            ...
                            return validRows.map((row) => {
                              const availableValues = row.attributeValues
                                .split(",")
                                .map((v) => v.trim())
                                .filter((v) => v);
                                ...
                                <div key={row.id} className="space-y-2">
                                  ...
                                  >
                                    <SelectTrigger className="border-gray-300 h-10 body-3 font-urbanist text-sm text-neutral-900">
                                      <SelectValue
                                        placeholder={`Choose ${row.attributeType}`}
                                      />
                                    </SelectTrigger>
                                    <SelectContent>
              ...
              {/* Add Similar Variant Button - Show when variants already exist */}
              {/* {showVariantsTable && createdVariants.length > 0 && (
                <div className="mt-4 bg-white rounded-lg border border-gray-200 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-semibold text-gray-900 font-urbanist">Add Similar Variant</h4>
                      <p className="text-xs text-gray-500 font-urbanist mt-1">Create a new variant by selecting attribute values</p>
                    </div>
                    <Button
                      type="button"
                      onClick={handleOpenDuplicateModal}
                      className="h-9 bg-secondary-900 hover:bg-secondary-800 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2"
                    >
                ...
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  ...
                  <div className="flex items-start">
                    <img
                      src={imagePreviewUrl}
                      alt={productName}
                      className="w-[56px] h-[56px] object-cover rounded-lg flex-shrink-0"
                    />
                    <div className="ml-3 flex-1">
                      <div className="text-[16px] text-gray-900 font-urbanist font-semibold">
                        {productName}
                      </div>
                    ...
                    <div className="text-right">
                      <div className="text-[16px] text-gray-900 font-spectral font-bold">
                        {formatIndianCurrency(productPrice || 0)}
                      </div>
                      <div className="body-3 text-gray-500 font-urbanist">
                        /unit
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    {(() => {
                      ...
                      return validRows.map((row) => (
                        ...
                        >
                          <div className="text-[14px] font-semibold body-3 text-gray-500 font-urbanist border-b border-gray-200 px-[8px] py-[6px]">
                            {row.attributeType}
                          </div>
                  ...
                  {/* Upload area */}
                  <div className="mt-4 border-2 border-dashed border-secondary-900 rounded-md p-6 text-center">
                    <div className="mx-auto mb-2 w-9 h-9 rounded-full flex items-center justify-center">
                      ...
                      >
                        <path d="M12 16V4" />
                        <path d="m8 8 4-4 4 4" />
                        <path d="M20 16.5a3.5 3.5 0 0 1-3.5 3.5h-9A3.5 3.5 0 0 1 4 16.5 3.5 3.5 0 0 1 7.5 13H9" />
                      </svg>
                    </div>
                    <div className="text-sm text-gray-600 font-urbanist">
                      Drag your file(s) or{" "}
                      <label
                        htmlFor="media-input-edit"
                        className="text-secondary-900 underline cursor-pointer"
                      >
                        browse
                      </label>
                    </div>
                    <div className="mt-1 text-xs text-gray-500 font-urbanist">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleFileUpload(selectedVariantIndex, e.target.files)
                      }
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-4">
                    {/* Display API images first */}
                    {(variantImageUrls[selectedVariantIndex] || []).map(
                      (url, idx) => (
                        <div
                          key={`api-${idx}`}
                        ...
                        >
                          ...
                          <button
                            type="button"
                            className="absolute top-2 right-2 w-6 h-6 bg-white border rounded-full flex items-center justify-center hover:bg-red-50"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDeleteVariantApiImage(selectedVariantIndex, idx);
                            }}
                          >
                            <X className="h-3 w-3 text-gray-700" />
                          </button>
                          <div className="mt-2 text-[12px] text-gray-800 truncate font-urbanist">
                            API Image {idx + 1}
                          </div>
                          <div className="text-[12px] text-gray-500 font-urbanist">
                            From Server
                          </div>
                        </div>
                      )
                    ...
                    {(createdVariants[selectedVariantIndex]?.media || []).map(
                      (file, idx) => {
        ...
        <DialogContent className="sm:max-w-[500px] bg-white">
          ...
          <div className="py-4">
            <Label
              ...
              className="body-2 font-semibold text-gray-700 font-urbanist mb-2 block"
            >
              Attribute Name
            </Label>
            <Input
          ...
          {selectedAdditionalDetailsIndex !== null && (
            <div className="py-4 space-y-4">
              ...
              <div className="space-y-2">
                <Label
                  htmlFor="extra-lead-time"
                  className="body-2 font-semibold text-gray-700 font-urbanist"
                >
                ...
                <Input
                  ...
                  inputMode="decimal"
                  value={
                    createdVariants[selectedAdditionalDetailsIndex]
                      ?.extraLeadTime === 0
                      ? ""
                      : createdVariants[selectedAdditionalDetailsIndex]
                          ?.extraLeadTime || ""
                  ...
                  onChange={(e) => {
                    ...
                    if (value === "" || /^\d*\.?\d*$/.test(value)) {
                      setCreatedVariants((prev) =>
                        prev.map((v, i) =>
                          i === selectedAdditionalDetailsIndex
                            ? {
                                ...v,
                                extraLeadTime:
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  <Input
                    ...
                    placeholder="Length"
                    className="border-gray-300 h-10 body-3 font-urbanist text-sm"
                  />
                  <Input
                    type="text"
                    inputMode="decimal"
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      setCreatedVariants((prev) =>
                        prev.map((v, i) =>
                          i === selectedAdditionalDetailsIndex
                            ? { ...v, variantDimensionUnit: value }
                            : v
                        )
                      );
                    }}
                  >
                    <SelectTrigger className="border-gray-300 h-10 body-3 font-urbanist text-sm">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
      ...
      <Dialog open={isDuplicateModalOpen} onOpenChange={handleCloseDuplicateModal}>
        <DialogContent className="sm:max-w-[500px] bg-white">
          <DialogHeader>
            <DialogTitle className="label-1 font-semibold text-gray-900 font-urbanist flex items-center gap-2">
          ...
          <div className="py-4 space-y-4">
            <p className="text-sm text-gray-600 font-urbanist">
              Select attribute values to create a new variant. Other details will be copied from the source variant.
            </p>
              ...
              return validRows.map((row) => {
                ...
                return (
                  <div key={row.id} className="space-y-2">
                    ...
                    <Select
                      value={duplicateVariantSelections[row.attributeType] || ""}
                      onValueChange={(value) => {
                        setDuplicateVariantSelections((prev) => ({
                          ...prev,
                          [row.attributeType]: value,
                        }));
                      }}
                    >
                      <SelectTrigger className="border-gray-300 h-10 body-3 font-urbanist text-sm text-neutral-900">
                        <SelectValue placeholder={`Select ${row.attributeType}`} />
                      </SelectTrigger>
                      <SelectContent>
                        {availableValues.map((value) => (
                          <SelectItem key={value} value={value}>
            ...
            {/* Preview of the new variant combination */}
            {(() => {
              const validRows = attributeRows.filter(
                (row) =>
                  row.attributeType && row.attributeValues && row.attributeValues.trim()
              ...
              if (allSelected && validRows.length > 0) {
                const variantParts = validRows.map(
                  (row) => duplicateVariantSelections[row.attributeType]
                );
                const previewCombination = variantParts.join(" / ");
                ...
                return (
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="text-sm text-gray-800 font-urbanist font-medium">
                      {previewCombination}
                    </div>
                  </div>
                );
          ...
          <DialogFooter className="flex gap-2">
            ...
            <Button
              type="button"
              onClick={handleAddDuplicateVariant}
              disabled={(() => {
                const validRows = attributeRows.filter(
                  (row) =>
                    row.attributeType && row.attributeValues && row.attributeValues.trim()
...
export default ProductOptionsVariantsAdmin;
