...
import { X, Plus, Info, ChevronDown, ChevronUp, User, CirclePlus, ChevronLeft, ChevronRight, CloudUpload, Settings, Search, Copy, Loader2 } from 'lucide-react'
import type { 
  VariantOption, 
  VariantOptionValue,
  Variant,
...
export const ProductOptionsVariants = ({ 
  ...
  const [variantImageUrls, setVariantImageUrls] = useState<Record<number, string[]>>({}) // Store API image URLs
  ...
  const [extraChargesByCombination, setExtraChargesByCombination] = useState<Record<string, number>>({})
      ...
      if (createdVariants.length > 0 && validRows.length > 0) {
        ...
        const formattedVariants = createdVariants.map((variant, variantIndex) => {
          ...
          return {
            ...
            is_active: (variant.isActive !== undefined ? variant.isActive : true) ? "1" : "0",
            images,
  ...
  React.useEffect(() => {
    ...
    const hasInvalidVariants = createdVariants.some(variant => {
      ...
      const variantAttributeTypes = Object.keys(variant).filter(key => 
        !['id', 'productName', 'extraCharge', 'stock', 'media', 'apiId', 'isActive', 'extraLeadTime', 
    ...
    if (hasInvalidVariants) {
      ...
      const cleanedVariants = validVariants.map(variant => {
        ...
        Object.keys(cleaned).forEach(key => {
          if (!['id', 'productName', 'extraCharge', 'stock', 'media', 'apiId', 'isActive', 'extraLeadTime', 
                'variantLength', 'variantWidth', 'variantHeight', 'variantDimensionUnit',
                'variantWeight', 'variantWeightUnit'].includes(key) && 
  ...
  React.useEffect(() => {
    if (apiVariants && apiVariants.length > 0) {
      ...
      optionNames.forEach((optionName, index) => {
        ...
        newAttributeRows[index].attributeValues = Array.from(uniqueValues).join(', ')
      })
      ...
      //   extraLeadTime: formattedVariants[0]?.extraLeadTime,
      //   variantDimensions: `${formattedVariants[0]?.variantLength}x${formattedVariants[0]?.variantWidth}x${formattedVariants[0]?.variantHeight} ${formattedVariants[0]?.variantDimensionUnit}`,
      //   variantWeight: `${formattedVariants[0]?.variantWeight} ${formattedVariants[0]?.variantWeightUnit}`
      // })
    }
  }, [apiVariants])
  ...
  const handleFileUpload = async (index: number, files: FileList | null) => {
    ...
    setCreatedVariants(prev => prev.map((v, i) => (i === index ? { ...v, media: [...(v.media || []), ...validFiles] } : v)))
    // Previews - append new URLs to existing previews
    const urls = validFiles.map(f => URL.createObjectURL(f))
    setMediaPreviews(prev => ({ ...prev, [index]: [...(prev[index] || []), ...urls] }))
  ...
  const filteredVariants = React.useMemo(() => {
    ...
    return createdVariants.filter(variant => {
      ...
      for (const row of validRows) {
        ...
        if (attributeValue && attributeValue.toString().toLowerCase().includes(query)) {
          return true
        }
      }
          ...
          <CollapsibleContent>
            <CardContent className="px-0 pb-0">
                      ...
                      <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Media <span className="text-red-500">*</span></th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {paginatedVariants.map((variant, index) => {
                            // Find the actual index in createdVariants array by matching the variant
                            ...
                            <tr key={variant.id || index} className="hover:bg-gray-50">
                                  ...
                                  onBlur={() => {
                                    ...
                                    setCreatedVariants(prev => {
                                      ...
                                      if (variant) {
                                        ...
                                        const combinationKey = getVariantCombinationKey(variant, validRows)
                                        if (numValue > 0) {
                              ...
                              <td className="px-4 py-3 text-sm">
                                {((variant.media && variant.media.length > 0) || (variantImageUrls[actualIndex] && variantImageUrls[actualIndex].length > 0)) ? (
                                  <button 
                                    type="button"
                              ...
                              <td className="px-4 py-3 text-sm">
                                <Button 
                                  ...
                                  onClick={() => {
                                    setCreatedVariants(prev => 
                                      prev.map((v, i) => 
                                        i === actualIndex ? { ...v, isActive: !v.isActive } : v
                                      )
                                    )
                                  }}
                                >
                                  {variant.isActive ? 'Activated' : 'Inactive'}
                                </Button>
                              </td>
                              <td className="px-4 py-3 text-sm">
                                <div className="flex items-center gap-2">
                                  <Button 
                                    ...
                                    variant="outline"
                                    size="sm"
                                    className="h-7 w-7 p-0 border-gray-300 hover:bg-secondary-50"
                                  ...
                                  <Button 
                                    ...
                                    onClick={() => {
                                      ...
                                      if (variantToDelete) {
                                        ...
                                        const combinationKey = getVariantCombinationKey(variantToDelete, validRows)
              ...
              <div className="text-center py-[30px]">
                <p className="text-gray-400 text-sm font-medium">No variants created yet</p>
                <Button 
                  type="button"
                  variant="outline"
                  size="sm"
                  className="h-8 px-3 font-medium bg-secondary-900 hover:bg-secondary-800 text-white mt-2 hover:text-white"
                  onClick={() => setIsSlideOpen(true)}
                >
                  Create Variants
                </Button>
                ...
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="text-right">
                      <div className="text-[16px] text-gray-900 font-spectral font-bold">{formatIndianCurrency(productPrice || 0)}</div>
                  ...
                  <div className="mt-4 border-2 border-dashed border-secondary-900 rounded-md p-6 text-center">
                    <div className="mx-auto mb-2 w-9 h-9 rounded-full flex items-center justify-center">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 text-"><path d="M12 16V4"/><path d="m8 8 4-4 4 4"/><path d="M20 16.5a3.5 3.5 0 0 1-3.5 3.5h-9A3.5 3.5 0 0 1 4 16.5 3.5 3.5 0 0 1 7.5 13H9"/></svg>
                    </div>
                    <div className="text-sm text-gray-600 font-urbanist">Drag your file(s) or <label htmlFor="media-input-edit" className="text-secondary-900 underline cursor-pointer">browse</label></div>
                    <div className="mt-1 text-xs text-gray-500 font-urbanist">Max 10 MB files are allowed</div>
                    <input id="media-input-edit" type="file" className="hidden" multiple accept="image/jpeg,image/jpg,image/png,image/webp" onChange={(e)=>handleFileUpload(selectedVariantIndex, e.target.files)} />
                  </div>

                  {/* Previews */}
                  <div className="grid grid-cols-3 gap-3 mt-4">
                    {/* Display API images first */}
                    {(variantImageUrls[selectedVariantIndex] || []).map((url, idx) => (
                      <div key={`api-${idx}`} className="relative border border-gray-200 rounded-lg p-2 bg-white">
                        <img 
                          src={url} 
                          alt={`Image ${idx + 1}`} 
                        ...
                        <div className="mt-2 text-[12px] text-gray-800 truncate font-urbanist">API Image {idx + 1}</div>
                        <div className="text-[12px] text-gray-500 font-urbanist">From Server</div>
                      </div>
                    ))}
                    ...
                    {(createdVariants[selectedVariantIndex]?.media || []).map((file, idx) => {
                      ...
                      return (
                        <div key={`file-${idx}`} className="relative border border-gray-200 rounded-lg p-2 bg-white">
                          <img 
                            src={src} 
                            alt={file.name} 
                            className="w-full h-32 object-cover rounded cursor-pointer hover:opacity-80 transition-opacity"
        ...
        <DialogContent className="sm:max-w-[500px] bg-white">
          ...
          <div className="py-4 space-y-4">
            ...
            {(() => {
              ...
              if (allSelected && validRows.length > 0) {
                const variantParts = validRows.map(row => duplicateVariantSelections[row.attributeType])
